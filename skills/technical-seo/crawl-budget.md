...
- Server log data or crawl log export (CSV or pasted text) showing Googlebot activity
- Optionally: robots.txt contents, GSC Coverage report data, site URL and CMS type, total number of indexable pages

Output Instructions:
Produce a structured crawl budget audit with severity-labeled issues, an action plan table, and a summary of estimated crawl efficiency improvement if all fixes are applied.
