...
- Product name(s)
- Product details: price, currency, availability, brand, SKU, GTIN/MPN (if known)
- Product image URL(s)
- Page URL for each product
- Ratings and review data (if available)
- Optionally: e-commerce platform in use, number of products needing schema, and any custom attributes

Output Instructions:
Once the product details are provided, produce the JSON-LD schema block(s), the rich result eligibility checklist per product, and the platform template recommendation if applicable.
