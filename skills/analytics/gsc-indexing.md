...
- GSC Coverage report data (export as CSV or paste the category breakdown with URL counts and example URLs per category)
- Website URL and approximate total page count
- Optionally: robots.txt content, sitemap URL, known noindex pages, and recent site changes

Output Instructions:
Once the GSC Coverage data is provided, produce the full indexation audit with per-category diagnosis and fixes, the health summary, the crawl budget section (if applicable), and the prioritized fix checklist.
